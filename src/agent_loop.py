    ...
    if not suppress_local_context and not suppress_skills:
        try:
            ...
            if last_user and _skills_on:
                ...
                # Do not include the full skill INDEX here. It is user-editable
                # context, and unrelated entries add prompt-injection surface
                # while unnecessarily tainting ordinary mutations. Matched
                # skills remain wrapped as untrusted context below.
                if relevant_skills:
                    _skills_text = "\n".join(lines)
...
async def stream_aci_runtime(
    ...
    # Deterministic schema resolution may bypass model arbitration, but the
    # resolver is selected from the canonical contract rather than a domain
    # branch in this runtime.
    if _aci_enabled and _aci_mode == "aci" and _aci_fast_path_block is None:
        from src.field_resolvers import deterministic_action_for_contract
        resolved_action = deterministic_action_for_contract(
            _intent.get("resolved_contract"),
            query=_last_user,
            frame=_intent.get("intent_frame") or {},
            disabled_tools=disabled_tools,
        )
        if resolved_action:
            binding, payload = resolved_action
            _aci_fast_path_block = ToolBlock(binding, json.dumps(payload, sort_keys=True))
            _record_aci_framework("deterministic_schema_action")
