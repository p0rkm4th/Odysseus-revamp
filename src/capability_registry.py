    ...
    "homelab.manage": CapabilitySpec(
        ...
        actions=_actions(*(
            ActionSpec(
                ...
                execution_location=("host_broker" if action in {"execute_network_discovery", "execute_diagnostic_install"} else "application"),
                target_scope=("private_network" if action in {"plan_network_discovery", "execute_network_discovery"} else None),
                requires_direct_container_access=(action not in {"plan_network_discovery", "execute_network_discovery", "execute_diagnostic_install"}),
                target_resources=("network:private_scope",) if action in {"plan_network_discovery", "execute_network_discovery"} else (),
                locks=(("network:private_scope",) if action == "execute_network_discovery" else (("host:package_manager",) if action == "execute_diagnostic_install" else ())),
                rollback_capability="none",
                precheck_actions=("plan_service_restart",) if action == "execute_service_restart" else (("plan_diagnostic_install",) if action == "execute_diagnostic_install" else ()),
                ...
                verification=("service_active",) if action == "execute_service_restart" else (("observations_persisted", "network_map_reconciled") if action == "execute_network_discovery" else (("prerequisites_verified",) if action == "execute_diagnostic_install" else ())),
                state_invalidations=("service.status", "service.uptime", "service.process_start_time") if action == "execute_service_restart" else (("network.observations", "network.map") if action == "execute_network_discovery" else (("capability.health", "executable.availability") if action == "execute_diagnostic_install" else ())),
                risk_level=("high" if action in {"execute_network_discovery", "execute_service_restart", "execute_diagnostic_install"} else ("low")),
                idempotency=("conditional_retry" if action in {"execute_service_restart", "execute_diagnostic_install"} else ("replay_safe" if action == "execute_network_discovery" else "unknown")),
            ) for action in (
