    ...
    "homelab.manage": CapabilitySpec(
        ...
        actions=_actions(*(
            ActionSpec(
                ...
                precheck_actions=("plan_service_restart",) if action == "execute_service_restart" else (),
                postconditions=("service_active",) if action == "execute_service_restart" else (),
                verification=("service_active",) if action == "execute_service_restart" else (("observations_persisted", "network_map_reconciled") if action == "execute_network_discovery" else ()),
                state_invalidations=("service.status", "service.uptime", "service.process_start_time") if action == "execute_service_restart" else (("network.observations", "network.map") if action == "execute_network_discovery" else ()),
                risk_level=("high" if action in {"execute_network_discovery", "execute_service_restart"} else ("low")),
                idempotency=("conditional_retry" if action == "execute_service_restart" else ("replay_safe" if action == "execute_network_discovery" else "unknown")),
            ) for action in (
