...
| OSINT | present policy/research store | `research.public_sources` / `manage_osint`; visible intake uses existing `/api/research/start` and owner-scoped detail; existing upload/document/VL processors provide bounded attachment evidence | first-class OSINT window, tabs, intake, bounded owner-checked file/image/document evidence intake, clickable evidence dossier with source counts, report/findings, explicit fact/inference boundary, command/sidebar navigation; graph/timeline remain partial | focused surface + research/security/attachment regression | browser gate passed for navigation/intake/mobile; live provider run pending | attachment text is capped, explicitly tainted, and never promoted to canonical facts; corrections, canonical fact/inference ledger, and bounded delta research remain |
