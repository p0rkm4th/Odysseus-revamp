...
| Durable Runs / Action Contracts | present (src/work_engine.py, core/work_models.py, src/run_planner.py) | work.run.read, work.run.manage, preview/validate/replay projections | partial; execution lifecycle/invalidation pending | focused Work/planner/registry tests | Run dossier preview dogfood added; live consequential execution pending | additive Run lifecycle, structured owner-scoped preview, deterministic validation, knowledge-gap surfacing, Action target/lock/risk/retry/rollback/verification contract, lock recovery, and epistemic/temporal claim projection; execution lifecycle, invalidation, compensation, and richer inspector remain |
