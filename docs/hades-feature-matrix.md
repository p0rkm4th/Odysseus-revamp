...
| Missions / Watches | present via WorkGoal projection and existing Monitor engine | `/api/work/missions` projection plus existing bounded Monitor routes | Control Center Mission dossier; Watch projection remains in Hades status surfaces | focused Mission/Monitor tests | synthetic projection only; live restart/dedupe dogfood pending | Missions reuse Goals/Runs and Watches reuse Monitors; response execution and richer lifecycle persistence remain |
