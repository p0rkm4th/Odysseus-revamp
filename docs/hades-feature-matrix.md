...
| World Model relationships | present (core/work_models.py, src/world_model.py) | owner-scoped Work projection; CMDB/domain references | API projection; focused UI pending | focused relationship/traversal tests | synthetic dependency graph dogfood complete | evidence-backed typed edges, bounded neighbors, confidence/status filtering, and blast-radius projection; CMDB adapters, temporal graph UI, and broader domain ingestion remain |
