...
| Missions / Watches | present via WorkGoal projection and existing Monitor engine | `/api/work/missions` projection includes lifecycle, constraints, budget, allowed capabilities, checkpoints, and blockers; bounded Monitor routes expose response policy | Control Center Mission dossier and Watch projection | focused Mission/Monitor tests | synthetic projection only; live restart/dedupe dogfood pending | Missions reuse Goals/Runs and Watches reuse Monitors; response execution and richer lifecycle persistence remain; projections do not grant authority |
| Execution nodes / fabric | present as owner-scoped registry around current broker boundary | register, heartbeat, list, deterministic requirements selection with rejection reasons and resource requirements; no authority grant | Control Center Execution Nodes inspection shows health, trust, heartbeat, and unchanged broker authority | focused owner-scope/eligibility/migration/UI tests | synthetic registry only; live broker-node heartbeat pending | current broker remains canonical executor; sandboxing and scheduling migration remain |
| Delegated capability grants | present over WorkAction approval/digest state | exact Run/Action/capability/target/digest grants with expiry, revocation, and call limits | Control Center Delegated Grants inspection; trusted-caller binding narrowing | focused fail-closed scope/owner/replay/binding tests | synthetic grant dogfood only | grants are not credentials and cannot widen authority; full orchestrator grant issuance remains deliberate/future |
...
| Incidents / Changes | present (core/incident_models.py, src/incident_change.py) | Work/Run references; no second executor; Change preview includes planner validation/blast radius | Control Center tabs with owner-scoped Incident/Change dossiers, linked verified Run state, and blast radius | focused owner-scope, hypothesis, preview, reconciliation, dossier, and migration tests | synthetic incident/change records dogfood complete | hypothesis/evidence history and Change preview reference canonical Runs; remediation execution and live safe-service dogfood remain |
