...
| Durable Runs / Action Contracts | present (src/work_engine.py, core/work_models.py) | work.run.read, work.run.manage, existing domain actions | partial; lifecycle/contract projection pending | focused Work/registry tests | unit dogfood complete; migration/full-runtime pending | additive Run lifecycle, intent/plan/assumptions/checkpoints, non-mutating preview, Action target/lock/risk/retry/rollback/verification contract, owner-scoped lock collision prevention/recovery, and epistemic/temporal claim projection; Run UI/event reconstruction remain |
