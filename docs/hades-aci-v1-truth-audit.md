...
| Pushed source | `c0a281f5` | `origin/hades-aci-v1` |
| Deployed source | `c0a281f5` | `/api/version`, immutable image marker |
| Running image | `odysseus:candidate-c0a281f5` | Docker inspect |
...
| Framework-output classification | latest run reports `hades_framework_generated_fallback`; Qwen provider tokens `3`, Hades fallback chars `99`; one model call, zero tools | benchmark separates framework prose from provider output |
