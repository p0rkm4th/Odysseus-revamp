...
| Work / Projects / Tasks | Work Engine, Runs, Actions, `read_work` | IMPLEMENTED / LIVE_VERIFIED / PARTIAL | overview, attention, continuation and persistence tests, empty-state authenticated browser read with final answer and reload-safe delivery | promote one verified ACI mutation binding for project/task creation, then add cross-suite remediation/task journeys; current HTTP/UI mutations are not chat evidence | Tier 1 next |
