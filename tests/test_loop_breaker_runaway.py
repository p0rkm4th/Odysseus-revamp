...
from src.aci import detect_runaway_call
...
def test_distinct_batch_is_not_runaway():
    ...
    sigs = [f'manage_calendar:{{"action":"create_event","summary":"Birthday {n}"}}'
            for n in range(18)]
    assert detect_runaway_call(_freq(sigs)) is None
...
def test_identical_call_repeated_is_runaway():
    ...
    assert detect_runaway_call(_freq(sigs)) == 'manage_calendar'
...
def test_threshold_is_configurable():
    ...
    assert detect_runaway_call(_freq(sigs), threshold=5) == 'web_search'
    assert detect_runaway_call(_freq(sigs), threshold=6) is None


def test_empty_is_not_runaway():
    assert detect_runaway_call(collections.Counter()) is None
