...
def test_projection_has_no_duplicate_conflicting_bindings():
    assert set(TOOL_BINDINGS) == {"manage_assets", "privileged_action", "manage_homelab", "manage_osint", "manage_security_assessment", "read_memory", "manage_memory", "read_work", "manage_work", "manage_notes", "manage_tasks", "read_household", "read_recipes", "manage_recipes", "read_setup", "read_career", "read_communications", "developer_read", "web_search", "web_fetch"}
    assert len({binding.capability_id for binding in TOOL_BINDINGS.values()}) == 19
    for name, binding in TOOL_BINDINGS.items():
