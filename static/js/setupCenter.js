...
async function load(el) {
  ...
  try {
    ...
    const categories=Object.entries(projection.categories||{}).map(([category,modules])=>`<section class="setup-center-category"><h3>${esc(category)}</h3><div class="hades-record-list">${modules.map(module=>`<article class="hades-record-card setup-center-module"><div><strong>${esc(module.title)}</strong><p>${esc(module.description)}</p><small>${esc(module.status_reason)} · dependencies: ${esc((module.dependencies||[]).join(', ')||'none')}</small></div><div>${statusBadge(module.status,module.status==='CONFIGURED'?'success':module.status==='NEEDS_ATTENTION'||module.status==='DEGRADED'?'warning':'info')}${module.id==='communications.telegram'?'<button type="button" class="setup-health-button" data-health-id="communications.telegram">Validate</button>':''}<button type="button" class="setup-skip-button" data-setup-id="${esc(module.id)}" data-setup-status="${module.status==='SKIPPED'?'NOT_CONFIGURED':'SKIPPED'}">${module.status==='SKIPPED'?'Resume':'Skip'}</button></div></article>`).join('')}</div></section>`).join('');
