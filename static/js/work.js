...
async function openWorkEntity(kind, id, title) {
  ...
  if (kind === 'run') {
    let preview = null, validation = null;
    try { [preview, validation] = await Promise.all([api(`/api/work/runs/${encodeURIComponent(id)}/preview`), api(`/api/work/runs/${encodeURIComponent(id)}/validate`, {method:'POST', body:'{}'})]); } catch (e) { validation = {valid:false, failures:[{code:'preview_unavailable', message:e.message}]}; }
    ...
    body.innerHTML = `<div><div class="work-header"><div><h2>${esc(title)}</h2><p>Status: ${esc(row.status || '—')} · Lifecycle: ${esc(row.lifecycle_state || '—')}</p></div><span class="hades-badge ${validation?.valid ? 'hades-badge-success' : 'hades-badge-warning'}">${validation?.valid ? 'Validated' : 'Needs review'}</span></div><section class="work-grid"><section><h3>What Hades intends to do</h3><p>${esc(JSON.stringify(preview?.objective || row.intent || {}, null, 2))}</p><h3>Actions</h3>${actions}</section><section><h3>Reads / writes</h3><p>Reads</p>${list(preview?.reads)}<p>Writes</p>${list(preview?.writes)}<h3>Resources / locks</h3>${list(preview?.resources)}</section><section><h3>Assumptions / unknown state</h3>${list(preview?.assumptions)}${list(preview?.knowledge_gaps, 'No stale or unknown prerequisites.')}</section><section><h3>Approvals</h3>${list(preview?.approvals)}<h3>Verification</h3>${list(preview?.verification ? [preview.verification] : [])}</section><section><h3>Validation</h3>${validation?.valid ? '<p>Plan is structurally valid.</p>' : list(validation?.failures)}${list(validation?.warnings)}</section></section><details><summary>Canonical Run record</summary><pre>${esc(JSON.stringify(row, null, 2))}</pre></details>${row.task_id ? `<button class="work-related-task" data-id="${esc(row.task_id)}">Open task</button>` : ''}</div>`;
