...
async function load(el, focus='', filters={}) {
  ...
  try {
    ...
    syncButton.onclick=async()=>{syncButton.disabled=true;syncButton.textContent='Syncing…';try{await api('/api/work/world/relationships/sync-cmdb',{method:'POST',body:'{}'});await load(el,focus,filters);}catch(error){syncButton.disabled=false;syncButton.textContent='Sync CMDB';uiModule.showError(error.message);}};
