...
async function load(el, focus='', filters={}) {
  ...
  try {
    ...
    const relationOptions=['','RUNS_ON','DEPENDS_ON','USES','POINTS_TO','CONNECTED_TO','BACKED_UP_BY','OWNS'].map(value=>`<option value="${value}" ${filters.relation===value?'selected':''}>${value || 'All relations'}</option>`).join('');
