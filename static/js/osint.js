...
async function caseDossier(sessionId) {
  ...
  const claimCard = claim => {
    ...
    const state = claim.status === 'superseded' ? 'STALE' : kind;
    const contradictionCount = (claim.contradicting_references || []).length;
    return `<article class="work-card osint-claim-card"><div class="work-card-heading"><strong>${esc(claim.predicate || 'Claim')}</strong><span>${provenanceBadge(state)}</span></div><p>${esc(typeof claim.value === 'string' ? claim.value : JSON.stringify(claim.value || {}))}</p><small>${esc(claim.source || 'Unknown source')} · confidence ${esc(claim.confidence ?? '—')} · ${esc(claim.observed_at || claim.created_at || 'undated')}</small>${(claim.evidence_references || []).length ? `<div class="hades-provenance-row">${provenanceBadge('EVIDENCE')} ${esc(claim.evidence_references.join(', '))}</div>` : ''}${contradictionCount ? `<small class="muted">${contradictionCount} competing claim(s) recorded; inspect lineage in Evidence Explorer.</small>` : ''}</article>`;
  };
