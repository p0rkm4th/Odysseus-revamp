...
async function load(el) {
  ...
  try {
    const [integrations, permissions, plaid, connection, plaidConfig] = await Promise.all([
    ...
    body.innerHTML=`${moduleHeader({icon:'integrations',title:'Integration Center',description:'Connection health and capability readiness. Secrets and account numbers never appear here.',primary:'Refresh',primaryId:'integration-center-refresh'})}<p class="muted">${esc((permissions.policy_source||'Canonical policy services remain authoritative.'))}</p><section class="setup-center-category"><h3>Connected integrations</h3><div class="hades-record-list">${cards}</div></section><section class="setup-center-category"><h3>Finance / Plaid</h3><p class="muted">${esc(lifecycleMessage)}</p>${plaidSetup}<div class="hades-record-list">${plaidCards || '<p class="muted">No Plaid account connected.</p>'}</div><button type="button" class="integration-plaid-connect" id="integration-plaid-connect">${items.length ? 'Connect another account' : 'Connect Plaid'}</button><label class="integration-csv-import">Use a local bank export if Plaid is unavailable <input id="integration-finance-csv" type="file" accept=".csv,text/csv"></label><p class="muted">Accepts common exports with Date/Posted Date plus Amount, or Debit/Credit, and Merchant/Name/Description/Payee columns. CSV imports are owner-scoped snapshots, not live balances; repeated imports are safe.</p><p id="integration-plaid-status" class="muted" role="status"></p></section>`;
